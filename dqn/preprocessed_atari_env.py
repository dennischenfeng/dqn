...
class PreprocessedAtariEnv(gym.Env):
    ...
    def __init__(
        self, env: gym.Env, action_repeat: int = 4, clip_reward: bool = False, device: th.device = None
    ) -> None:
        """
        ...
        self.latest_obs_maxed_seq: List[np.ndarray] = []
    ...
    def reset(self) -> np.ndarray:
        ...
        obs_maxed = obs
        self.latest_obs_maxed_seq = [obs_maxed] * OBS_SEQUENCE_LENGTH
...
def preprocess_obs_maxed_seq(
    obs_maxed_seq: List[np.ndarray], preprocess_transform: Any, device: Optional[th.device] = None
) -> np.ndarray:
    """
    Preprocess the sequence of obs_maxed's, using `preprocess_transform`. Will first load onto `device`, to
